  ...
  return (
    <>
      ...
      <nav
        ...
        className="sticky top-16 z-30 border-b border-border bg-card/80 backdrop-blur-sm"
      >
        <div className="mx-auto flex max-w-7xl flex-wrap gap-2 px-4 py-3 sm:px-6 lg:px-8">
          {productCategories.map((cat) => (
            <Link
              key={cat.slug}
              href={`#${cat.slug}`}
              className="rounded-full border border-border bg-background px-4 py-1.5 text-sm font-medium text-foreground/80 transition-colors hover:border-primary/40 hover:bg-secondary hover:text-primary"
            >
              {cat.name}
            </Link>
          ))}
      ...
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="space-y-20">
          {productCategories.map((cat, i) => {
            ...
            const catProducts = cat.productSlugs.map(
                ...
                <div className={cn('grid items-start gap-10 lg:grid-cols-2')}>
                  <Reveal className={cn(reversed && 'lg:order-2')}>
                    <div className="group relative overflow-hidden rounded-2xl border border-border bg-secondary shadow-xl shadow-primary/8">
                      <Image
                        src={cat.image}
                        alt={`${cat.name} — manufactured by Jiangsu Changhui Electric`}
                        width={720}
                        height={540}
                  ...
                  <Reveal delay={100} className={cn(reversed && 'lg:order-1')}>
                    ...
                    <h2 className="mt-2 font-display text-2xl font-bold tracking-tight text-primary text-balance sm:text-3xl">
                      {cat.name}
                    </h2>
                      ...
                      {catProducts.map((prod) => (
                        <li key={prod.slug}>
                          <Link
                            ...
                            className="group flex items-center justify-between gap-3 rounded-lg border border-border bg-card px-4 py-3 text-sm font-medium text-foreground transition-colors hover:border-primary/40 hover:bg-secondary hover:text-primary"
                          >
                            <span>{prod.name}</span>
                    ...
                    <Link
                      ...
                      className="group mt-6 inline-flex items-center gap-2 rounded-md bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground transition-transform hover:-translate-y-0.5"
                    >
                      View {cat.name}
        ...
        <Reveal className="relative overflow-hidden rounded-3xl bg-primary px-6 py-14 text-center shadow-xl shadow-primary/20 sm:px-12">
          ...
          <div className="relative">
            ...
            <p className="mx-auto mt-3 max-w-xl text-white/80 text-pretty">
              We manufacture to order. Share your drawings or specifications and we will build a
              tailored solution.
            </p>
            <Link
              href="/contact"
